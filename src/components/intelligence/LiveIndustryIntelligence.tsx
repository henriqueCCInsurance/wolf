...
import { TrendingUp, BarChart3, Lightbulb, RefreshCw, Calendar, ExternalLink } from 'lucide-react';
import { industryIntelligence2025 } from '@/data/industryIntelligence2025';

interface LiveIndustryIntelligenceProps {
  industry: string;
  className?: string;
}

const LiveIndustryIntelligence: React.FC<LiveIndustryIntelligenceProps> = ({ 
  industry, 
  className = '' 
}) => {
  const [activeTab, setActiveTab] = useState<'stats' | 'trends' | 'insights'>('stats');
  ...
  const tabs = [
    { id: 'stats' as const, label: 'Live Stats', icon: BarChart3, count: industryData.keyStats.length },
...
export default LiveIndustryIntelligence;